...
const int task_processing_time_variance = 90;
/**
...
const int task_content_size_variance = 9;
...
int main() {
    ...
    for(int i = 0; i < n; ++i) {
		graph->add_edge(s, i, 0, query_size[i]);
    }
    ...
    for(int t = 1; t <= times; ++t) {
		...
		for(int i = 0; i < n; ++i) {
            for(int j = 0; j < query_size[i] * t / times; ++j) {
				tasks[cur]->current_pu = units[i];
				tasks[cur]->id = cur;
				units[i]->add_task(tasks[cur]);
				tasks[cur]->resolved = false;
				cur++;
	    ...
	    for(int i = 0; i < n; ++i) {
	    	units[i]->sch->reset();
	    	for(int j = 0; j < units[i]->outgoing_channels.size(); ++j) {
	    		units[i]->outgoing_channels[j]->reset();
	    		units[i]->outgoing_channels[j]->scheduled_load = floor(graph->get_flow(n + num) * tau * t / times + 0.5) * task_content_size_expectation;
