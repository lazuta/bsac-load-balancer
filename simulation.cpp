...
int main() {
    ...
    for(int i = 0; i < n; ++i) {
    	for(int j = 0; j < units[i]->outgoing_channels.size(); ++j) {
    		...
    		cerr << "Scheduled load: " << units[i]->outgoing_channels[j]->scheduled_load << " " << graph->get_flow(n + num) << endl;
    		num++; 
    	}
    }
	...
	*/
	...
	cerr << "Delay expected " << graph->longest_flow_path() << endl;
	if(graph->has_flow_cycle()) {
		cerr << "Graph has a flow cycle!!!" << endl;
	} else {
		cerr << "Graph has no flow cycles" << endl;
	}
