...
public:
	...
	/**
	 * Right now, a simple Edmonds-Karp algorithm is used. Needed to be improved to
	 * preflow later. Still good for simulation purposes.
	 */
	double max_flow() {
	...
	/**
	 * This function finds the maximum distance from any vertex to a vertex with
	 * the flow excess. Here for i'th vertex we call flow excess the value 
	 * sum_{j != s,t}flow(i, j) or simply flow(s, i) - f(i, t).
	 *
	 * This value is used when source and target nodes are abstract and the value
	 * represents network flow difference in the node.
	 *
	 * !!!TODO Actually works completely wrong!!!.
	 */
	int longest_flow_path() {
		if(d == NULL) {
			d = new int[n];
		}
		queue<int> q;
